...
struct Sender
{
  ...
  int window;
  int nBuffer;
  struct pkt packet_buffer[1024];
} S;

struct Receiver
{
  int exp_seq;
  struct pkt packet_send;
} R;
...
{
  while(S.nseq < S.nBuffer && S.nseq < S.base + S.window)
  {
    struct pkt *packet = &S.packet_buffer[S.nseq % 1024];
    printf(" Send window (seq=%d): %s\n",packet->seqnum, packet->payload);
    tolayer3(0, *packet);
    if(S.base == S.nseq)
    {
...
void A_output(struct msg message)
{
  ...
  if (S.nBuffer - S.base >= 1024)
  {
      printf("  Buffer full. drop the message: %s\n", message.data);  //throw an error
      return;
  }
  printf("  Bufferred packet (seq=%d): %s\n", S.nBuffer, message.data);
  struct pkt *packet = &S.packet_buffer[S.nBuffer % 1024];  //buffer the message

  packet->seqnum = S.nBuffer;
  memcpy(packet->payload, message.data, sizeof(message.data)); //copy over the message to paylaod

  packet->checksum = compute_check_sum(*packet);  //compute checksum
  ++S.nBuffer;  //increase buffer
  send_window();  //send the window
}

void B_output(struct msg message)
{
  /* If B is waiting, ignore the message */
	if (is_waiting_B)
		return;
...
{
  if (packet.checksum != compute_check_sum(packet)) //corrupted packet
  {
      printf("  Packet corrupted. drop.\n");
      return;
  }
  if (packet.acknum < S.base) //NAK because ack not in window
  {
      printf("  Got NAK (ack=%d). drop.\n", packet.acknum);
      return;
  }

  printf("  Got ACK (ack=%d)\n", packet.acknum);  //ack packet
  S.base = packet.acknum + 1; //increase base

  if (S.base == S.nseq) //got next expected message
  {
      stoptimer(0);
      printf("  Stop timer\n"); //stop timer and send window
      send_window();
  } else
  {
      starttimer(0, TIME_OUT);
...
{
  for (int i = S.base; i < S.nseq; ++i) //base is less than next seq number
  {
      struct pkt *packet = &S.packet_buffer[i % 1024];  //need to resend packet
      printf("  A_timerinterrupt: resend packet (seq=%d): %s\n", packet->seqnum, packet->payload);
      tolayer3(0, *packet);
  }
  starttimer(0, TIME_OUT);  //start new timer
...
void A_init()
{
  S.base = 1; //initialize values of sender
  S.nseq = 1;
  S.window = 8;
  S.nBuffer = 1;
}
...
struct pkt packet;
{
  if (packet.checksum != compute_check_sum(packet)) //packet corrupted?
  {
      printf("  B_input: packet corrupted. send NAK (ack=%d)\n", R.packet_send.acknum);
      tolayer3(1, R.packet_send); //resend
      return;
  }
  if (packet.seqnum != R.exp_seq) //out of order?
  {
  ...
  tolayer5(1, message); //send message to layer 5

  printf("  B_input: send ACK (ack=%d)\n", R.exp_seq);
  R.packet_send.acknum = R.exp_seq;
  R.packet_send.checksum = compute_check_sum(R.packet_send);
  tolayer3(1, R.packet_send); //send message ack
...
void B_init()
{
  R.exp_seq = 1;  //initialize reciever struct values
  R.packet_send.seqnum = -1;
  R.packet_send.acknum = 0;
  memset(R.packet_send.payload, 0, 20);
  R.packet_send.checksum = compute_check_sum(R.packet_send);
}
/*****************************************************************
